...
# We can use the command line to copy, move, and remove files
# and directories.

# When using commands, we can attach options to modify the 
# behavior of the commands.

# The following command displays the contents of the working
# directory in more detail; it'll display all the files and
# directories, including hidden ones (those whose names start
# with a dot character)
ls -a

# This will list all contents of a directory in long format, as
# well as the file permissions:
ls -l

# This will order files and directories by the time they were 
# last modified:
ls -t

# The -l option lists files and directories as a table. There are
# seven columns listed in the table:
...
# In addition to using each option separately, like ls -a or ls -l,
# mulitple options can be used together:
ls -alt
...
# and confirming the result of other commands that change the contents
# of files. 

# The cp command copies files or directories:
cp file_name.ext copy_name.ext

# We could also copy a file to a destination directory (keeps 
# the same name):
cp file_source.ext destination_directory

# Or we could further specify the name of the new copy in the 
# other folder:
cp file_source.ext destination_directory/new_name.ext

# To copy multiple files into a directory, use cp with a list
# of source files as the first arguments, and the destination
# directory as the last argument:
cp file1.ext file2.ext destination/directory

### Wildcards ###

# In addition to using exact filenames, we can use special characters
# like * to select groups of files. These special characters are 
# called wildcards. 
...
# Prefix text can also be used with wildcards. Using w*.txt selects
# all files in current directory starting with 'w' and ending with '.txt':
cp w*.txt different/directory

### Moving and Renaming ###

# Moving is similar to copying in its usage, except moving a file 
# does so without making a copy.

# To move a file into a directory:
mv old/path/to/file.ext new/path/to/file.ext

# To move multiple files into a directory, list the source files first
