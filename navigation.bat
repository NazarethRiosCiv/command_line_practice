...
:: # It's a program that takes in commands and passes them on
:: # to the computer's operating system to run.

:: # From the command line, you can navigate through files and
:: # folders on your computer, just like using the GUI tools like
:: # File Explorer on Windows or Finder on MacOS. 

:: # When using the command line, we refer to folders as directories.
:: # Files and directories on your computer are organized into a
:: # filesystem.

:: ### Filesystem ###

:: # A filesystem organizes a computer's files and directories
...
:: # called an argument. The cd command takes a directory name as an 
:: # argument and switches into that directory. 

:: # If you wish to move up one directory (the parent directory to the 
:: # working directory):
cd ..

:: # To move across multiple directories with a single command, we can 
:: # provide cd a relative path to the directory as an argument:
cd path\to\the\directory

:: # Note that for Windows systems, backward slash indicates pathing

:: # To go up from the working directory up two directories:
cd ..\..

:: # We can also move to an adjacent directory using .. and then
:: # a directory name:
cd ..\adj_directory
...
:: # The tab key can be used to autocomplete your command. When you
:: # are typing the name of an existing file or directory, you can 
:: # use tab to finish the rest of the name.

:: # The up and down keys can be used to cycle through your previous 
:: # commands. The up key will take you up through your most recent
:: # commands, and the down key will take you back through to the 
:: # most recent one. 
